...
- **Nested self-references are fetched from main, not the PR head — bootstrap pin required.** The dispatch substitutes the PR head SHA into the companion template, so the *top-level* reusable-workflow call runs the PR's code. But a wrangle reusable workflow that itself calls a wrangle composite action (`uses: TomHennen/wrangle/actions/<name>@<sha>`) is a **nested** self-reference: GitHub resolves it from its pinned SHA, which points at **main**, not the PR. So a PR that changes a composite action — or a file the action reads at runtime, e.g. a `policies/*.hjson` PolicySet — and wires that action into a reusable workflow under integration test will run the **old, main** action against inputs that only exist on the branch, and the integration test fails on code the PR didn't ship.
  - **Workaround (bootstrap pin):** point the *nested* self-ref at a branch SHA that carries the change, for the duration of the PR. Hand-edit just that one pin (leave unrelated pins on their main SHA so the post-merge dangling surface stays minimal), make the pin edit the **last** commit so it targets a SHA that already contains the action/policy, and state the bootstrap pin in the PR description. (First use: the `actions/verify` pin in `build_and_publish_python.yml`, PR for #247.)
  - **The post-merge danger is real and not human-paced — do NOT rely on "bump before the next release".** `release-showcase.yml` fires `on: push: branches:[main]` and auto-pushes a `vYYYYMMDD-<sha>` tracking tag on every code-bearing push to main, which drives the companion showcase to run `build_and_publish_python.yml@main` — exercising the nested pin **on the merge commit itself**, before anyone can re-bump. So the branch-SHA pin must stay **resolvable from main**:
    - **Merge this kind of PR as a merge commit, not a squash.** A merge commit keeps the branch-tip SHA an ancestor of main, so the pin resolves and the showcase stays green; the post-merge `tools/bump_action_pins.sh <main-sha>` re-bump then only corrects the comment label and can follow at leisure. A **squash** orphans the branch SHA the moment the branch is deleted — the pin no longer resolves and the auto-showcase (and any other main-side caller) breaks.
    - **Enforced, not remembered.** `tools/check_pin_ancestry.sh` (run in the `integration` CI job, which checks out with `fetch-depth: 0`) asserts every `TomHennen/wrangle/...@<sha>` pin under `.github/workflows/` is reachable from `HEAD`. It is green during the PR (the branch SHA is an ancestor of the branch) and **red on main** if a squash orphaned a pin — turning a silent degradation into a failed check that forces the re-bump.
