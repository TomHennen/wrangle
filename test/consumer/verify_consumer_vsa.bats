...
# --- adopter-side publish gate (actions/verify-artifact) ---
...
@test "verify-artifact: gate script verifies the real npm fixture end-to-end" {
    [[ -x "$COSIGN_BIN" ]] || skip_or_fail "real cosign not available"
    require_sigstore
    ...
    PATH="$(dirname "$COSIGN_BIN"):$PATH" \
        ...
        run "$REPO_ROOT/actions/verify-artifact/verify_artifact.sh"
    [[ "$status" -eq 0 ]]
    [[ "$output" == *"1 file(s) verified against PASSED VSAs"* ]]
}

@test "verify-artifact: gate script rejects the fixture under a wrong origin repo (fail-closed)" {
    [[ -x "$COSIGN_BIN" ]] || skip_or_fail "real cosign not available"
    require_sigstore
    mkdir -p "$TMP/dist" "$TMP/vsas"
    cp "$BLOB" "$TMP/dist/npm-package.tgz"
