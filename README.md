...
- **Provenance `builder.id` is the SLSA generator's, not wrangle's.** For npm/go/python, *wrangle's* reusable workflow runs the build and the *generic* generator only signs the provenance — so `builder.id` names the prov-signer, and the provenance binds the source repo but **not which workflow built the artifact** (a hole an attacker could exploit by handing the generic generator their own hashes — see [`docs/SPEC.md`](docs/SPEC.md)). Containers differ: the container generator isolates the build. *Being closed:* wrangle now **also** emits GitHub-issued provenance (`actions/attest-build-provenance`) whose `builder.id` and signing certificate are *wrangle's* reusable build workflow, verified in-run against its signer identity; it runs alongside the generator during the migration, after which the generator path is dropped and the VSA rebaselines onto it. Tracking: [#316](https://github.com/TomHennen/wrangle/issues/316).
