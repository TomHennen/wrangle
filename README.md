...
- **The verifiable "wrangle built + verified this" anchor is the VSA's signing certificate — not the `builder.id` or `verifier.id` fields.** The VSA `verifier.id` is `https://carabiner.dev/ampel@v1` (the ampel engine, hard-coded), and (above) `builder.id` is the generator. Neither names wrangle. But the VSA is keyless-signed by *wrangle's* reusable workflow (`…/TomHennen/wrangle/.github/workflows/build_and_publish_<type>.yml@…`) in the same run that built the artifact — so verifying that signing identity is what ties the artifact to wrangle's hardened build+verify, closing the gap above for npm/go. Pin wrangle's workflow path, not the engine and not your own repo. Verify with `cosign verify-blob-attestation` + a `jq` predicate check (npm/go), or `ampel verify` against the wrangle-hosted consumer policy (any type — the only digest-native path for containers); see each build-type README. Note the standard `slsa-verifier verify-vsa` does **not** work here — it verifies only key-signed VSAs, and wrangle's are keyless. Tracking: [#295](https://github.com/TomHennen/wrangle/issues/295).
