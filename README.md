...
> [!NOTE]
> On a private repo the build pipelines can't attest yet (attestation would leak the repo to a public log), so they run in unattested mode via `attestation: disabled` — see [the FAQ](docs/FAQ.md#can-i-use-wrangle-on-a-private-repo).
