...
The reusable workflow runs an `attest:` job that calls `actions/attest-build-provenance` with `subject-path: dist/*`. The step runs *inside* wrangle's reusable workflow, which is the isolated trusted builder — so the provenance is non-falsifiable and names `build_and_publish_python.yml` as its `builder.id`. The job then assembles one `<artifact>.intoto.jsonl` bundle per subject (the provenance + that subject's signed metadata) and uploads the bundle directory as a transient workflow artifact (`internal-python-bundles-<shortname>`) that the `verify` job downloads to append each VSA; the completed bundles ship in the unified metadata artifact (`metadata-artifact-name`).
