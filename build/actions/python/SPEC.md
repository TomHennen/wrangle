...
| `provenance-artifact-name` | reusable workflow only | Name of the workflow artifact the per-subject `<artifact>.intoto.jsonl` bundles (SLSA provenance + that subject's signed metadata) ship in. They live inside the unified metadata artifact, so this equals `metadata-artifact-name` on release runs. Empty when `should-release` is false. |
...
The reusable workflow runs an `attest:` job that calls `actions/attest-build-provenance` with `subject-path: dist/*`. The step runs *inside* wrangle's reusable workflow, which is the isolated trusted builder — so the provenance is non-falsifiable and names `build_and_publish_python.yml` as its `builder.id`. The job then assembles one `<artifact>.intoto.jsonl` bundle per subject (the provenance + that subject's signed metadata) and uploads the bundle directory as a workflow artifact (`python-bundles-<shortname>`); the reusable workflow exports that name as `provenance-artifact-name` so the adopter can locate it.
...
The **showcase** (the post-merge, tag-driven companion workflow) publishes to **TestPyPI** (`https://test.pypi.org/legacy/`) rather than skipping publish entirely. This exercises the full publish path including Trusted Publishing OIDC exchange and PEP 740 attestation generation. The package name should be disposable (e.g., `wrangle-test-fixture`). A Trusted Publisher must be configured on test.pypi.org for the showcase workflow. The per-PR integration test covers build → provenance → verify (the wrangle-owned path) but does **not** publish: publishing is a release-time action, and npm Trusted Publishing allows only one trusted publisher per package, so a single workflow (the showcase) owns the publish slot.

Because TestPyPI rejects re-uploads of the same version, the fixture must produce a fresh version per run — typically by substituting a placeholder in `pyproject.toml`'s version, or by using a dynamic version backend like `hatch-vcs`.
