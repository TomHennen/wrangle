...
Wrangle drives your existing `.goreleaser.yml` to build your binaries, then adds the security drudgery around it: gofmt/vet/test/govulncheck, an SPDX SBOM, SLSA Build L3 provenance, and a signed VSA your users can verify with one command. Wrangle does **not** publish — you own that (the [example](../../../gh_workflow_examples/build_go.yml) shows a `publish` job that creates the GitHub Release), the same shape as the python and npm build types.

Goreleaser runs with `--skip=publish`, so its downstream verbs (Docker pushes, Homebrew taps, deb/rpm, announcements) do **not** run under wrangle. Those would ship bytes wrangle can't cover with provenance, which is exactly what an adopter who came here to "do the right thing" doesn't want silently mixed in. Run them from your own publish job, or — for attested images — use wrangle's [container build type](../container/README.md).
...
Push a `v`-prefixed semver tag (e.g. `v1.2.3`) and wrangle runs the full pipeline; the example's `publish` job then creates the release. PRs build and test without publishing.
...
- **You publish; wrangle attests what's in `checksums.txt`.** wrangle runs goreleaser with `--skip=publish` and attests the archives in `checksums.txt`. Your publish job ships those archives + their VSA bundles (see the [example](../../../gh_workflow_examples/build_go.yml)). Docker images, Homebrew taps, and deb/rpm in your `.goreleaser.yml` are *not* run by wrangle; run them from your own job, or use wrangle's [container build type](../container/README.md) for attested images.
