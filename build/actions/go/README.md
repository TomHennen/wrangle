...
- **When govulncheck goes red**, upgrade the affected module — or, for a stdlib finding, the `go` directive in your `go.mod`. If upstream has no fix, accept it in `govulncheck-ignore.toml` next to your `go.mod`: the failure message prints the exact `[[IgnoredVulns]]` stanza to paste, and you fill in the `reason` and a date to re-decide by. Need the whole check advisory-only? Pass `govulncheck: info`. Details in [`SPEC.md`](./SPEC.md) "Failure semantics".
