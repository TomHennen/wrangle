...
jobs:
  build:
    ...
    uses: TomHennen/wrangle/.github/workflows/build_and_publish_go.yml@v0.4.1 # zizmor: ignore[unpinned-uses] - immutable
    with:
      path: "."
```
...
- **Immutable releases work** — wrangle creates the release as a draft, attaches every asset, then publishes, so [immutable releases](https://docs.github.com/en/code-security/concepts/supply-chain-security/immutable-releases) accept it. Let wrangle own the release (keep goreleaser's `--skip=publish`); a release published for the tag before wrangle runs can't be amended.
...
- **When govulncheck goes red**, upgrade the affected module — or, for a stdlib finding, the `go` directive in your `go.mod`. If upstream has no fix, add an `[[IgnoredVulns]]` entry with a `reason` and an `ignoreUntil` to the `osv-scanner.toml` next to your `go.mod`; the same entry silences both scanners — whether you name the `GO-`, `CVE-` or `GHSA-` id for the advisory — and the expiry brings the finding back so you re-decide instead of forgetting. Need the whole check advisory-only? Pass `govulncheck: info`. Details in [`SPEC.md`](./SPEC.md) "Failure semantics".
...
ampel verify --subject <archive> \
  --policy git+https://github.com/TomHennen/wrangle@v0.4.1#policies/wrangle-vsa-consumer-v1.hjson \
