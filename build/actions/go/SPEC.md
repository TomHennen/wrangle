...
The JSON is written to `<metadata_dir>/govulncheck.json` whatever the verdict, and the reusable workflow uploads that directory as the `checks` transient even when the job fails — so a red gate still hands the adopter the traces. Folding it into the unified metadata happens in the `release` job, which a failing `checks` job blocks, so that only happens on the passing path.
...
wrangle reads `[[IgnoredVulns]]` entries — `id`, a required non-empty `reason`, and an optional `ignoreUntil` — from `govulncheck-ignore.toml` **beside the scanned `go.mod`** (i.e. `<path>/govulncheck-ignore.toml`), applying only to that directory's module; wrangle does not walk parent directories.

The schema is osv-scanner's `[[IgnoredVulns]]`, but the **file is deliberately not** `osv-scanner.toml`. The commonest legitimate reason to ignore an osv finding is "we don't call the vulnerable code" — precisely the claim govulncheck exists to test. A shared file would let that entry silence govulncheck on the very day it proved the claim false, which has already happened here once: `tools/osv-scanner.toml` carried GO-2026-6225 as "unreachable … never linked", and it was reachable. The two files record acceptances of different strength — *"a module I depend on has an advisory"* versus *"my code reaches a known-vulnerable symbol and I am shipping anyway"* — and the second earns its own sign-off. That is also why `reason` is required rather than optional here.

Matching is exact-string on the `GO-…` id govulncheck printed — no aliases, no prefixes, no patterns. An `ignoreUntil` that has passed stops suppressing, so an acceptance is re-decided rather than inherited; it accepts a bare date as well as a TOML datetime, and a value with no offset is read as UTC. When the gate fails it prints the exact stanza to add, with the id filled in, after telling the adopter to prefer a version bump.
