...
wrangle reads `[[IgnoredVulns]]` entries — `id`, plus an optional `ignoreUntil` — from the `osv-scanner.toml` **beside the scanned `go.mod`** (i.e. `<path>/osv-scanner.toml`), mirroring [osv-scanner's own config discovery](https://google.github.io/osv-scanner/configuration/): the config applies to the manifests in its own directory, and wrangle does not walk parent directories. One file therefore suppresses an advisory for both scanners, and the shared `GO-…` ids line up. An entry is matched against a finding's advisory id *and its aliases* — the `aliases` the scan's own `osv` records carry — because [osv-scanner ignores an advisory's aliases too](https://google.github.io/osv-scanner/configuration/), so an entry naming a `CVE-`/`GHSA-` id silences the `GO-` id govulncheck reports for the same advisory. Matching is exact-string, never prefix or pattern. An `ignoreUntil` that has passed stops suppressing, so a suppression is re-decided rather than inherited; `ignoreUntil` accepts osv-scanner's documented bare date as well as a TOML datetime, and a value with no offset is read as UTC.
