...
**Cosign keyless signing of binaries — alongside SLSA provenance, in scope.** The two signatures make distinct claims and ship together for binaries that are part of the user's trust path: SLSA provenance attests *"these bytes were produced by this workflow from this source"* (binding workflow identity + commit + builder), while `cosign sign-blob --yes` against each artifact attests *"this exact byte sequence is what we released"* (a registry-independent, point-and-verify signature on the artifact itself). Different verifier surfaces lean on each — `slsa-verifier verify-artifact` for the provenance, `cosign verify-blob` for the signature — and downstream consumers may verify one, the other, or both depending on their toolchain. This is the pattern goreleaser's own release pipeline uses and the ecosystem norm for security-critical Go binaries (`cosign`, `oras`, `slsa-verifier` itself). Identity is the calling workflow's OIDC token (same model as wrangle's container build type signing the image digest); the cosign step is in addition to, not a replacement for, the `generator_generic_slsa3.yml` provenance step.
