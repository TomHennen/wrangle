...
15. Verify the provenance and emit the signed SLSA VSA (ampel against wrangle-provenance-container-v1; gated on should-release); deliver the bundle as a workflow artifact + release asset (when a release exists), and push it to the registry as a best-effort OCI referrer (cosign attach attestation)
...
Step 15's VSA *delivery* matches the npm/Go/Python flow: the signed VSA rides in a per-artifact `.intoto.jsonl` bundle, uploaded as a workflow artifact always and attached to the GitHub release when the image is tag-published. Container *additionally* pushes the bundle to the registry as a best-effort OCI referrer on the image digest (`cosign attach attestation`, which uploads the already-bnd-signed bundle verbatim — it does not re-sign). `cosign attach attestation` accepts a single per-artifact bundle line and round-trips it through `cosign download attestation` with its `verificationMaterial` intact, but rejects a multi-line concatenation — so each bundle is pushed as its own referrer, and because the file delivery is the guaranteed path a referrer-push failure is logged and swallowed, never failing the job. Step 15 also differs in how ampel *reads* the build provenance it evaluates: the npm/Go/Python flows feed ampel the attest job's local Sigstore bundle via the `jsonl:` collector, but the container provenance lives only in the registry as an OCI referrer, so the container verify job uses the `oci:` collector to list the image's referrers and select the signed `attest-build-provenance` bundle. `cosign attach` runs inside `actions/verify` in the same process as emit+sign, so the unsigned VSA never crosses a step boundary; the verify job logs in to ghcr first and requests `packages: write` (provenance pull + best-effort referrer push) and `contents: write` (attach the bundle to the release on a tag).
...
| VSA file delivery (workflow artifact / release asset) | **Pipeline stops** (fail-closed) | The bundle is the consumer trust artifact; the workflow-artifact upload (`if-no-files-found: error`) and the release attach are the guaranteed delivery, so a failure there fails the verify job and, via `needs:` propagation, the workflow. |
| VSA registry-push (OCI referrer) | **Continue** (best-effort) | The referrer is a retrieve-by-digest convenience on top of the file delivery; `cosign attach attestation` can fail on a registry/auth blip, so a failure is logged and swallowed rather than failing the job — the consumer can always fetch the file bundle. |
...
| `contents` | `write` | Attach the per-artifact bundle to the GitHub release for the tag, when one exists |
| `packages` | `write` | Pull the image's provenance for the ampel collector **and** push the best-effort VSA referrer to the registry |

The container VSA is delivered as a file (workflow artifact always, release asset when the image is tag-published), like npm/Go/Python, so the verify job needs `contents: write` to attach to the release. The container caller must grant `contents: write` for the attach to succeed; on a non-tag run the attach step is skipped, but the scope is still requested so a tag-publishing adopter's release attach works.
...
The signed SLSA VSA (`predicateType: https://slsa.dev/verification_summary/v1`) rides in a per-artifact `<image-basename>-<digest>.intoto.jsonl` bundle. The **guaranteed** delivery is the file bundle: a workflow artifact on every run, and a GitHub release asset when the image is tag-published (npm/Go/Python deliver the same way). As a retrieve-by-digest convenience, container builds **additionally** push the bundle to the **registry as a best-effort OCI referrer** on the image digest — so consumers can fetch it from just the image reference when the push succeeded.

When the referrer is present, consumers retrieve it from the image reference:
