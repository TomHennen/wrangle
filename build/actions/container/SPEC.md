...
15. Verify the provenance and emit the signed SLSA VSA (ampel against wrangle-provenance-container-v1; gated on should-release); deliver the bundle as a workflow artifact and push it to the registry as a best-effort OCI referrer (cosign attach attestation)
...
| VSA registry-push (OCI referrer) | **Continue** (best-effort) | The referrer is a retrieve-by-digest convenience on top of the workflow artifact; `cosign attach attestation` can fail on a registry/auth blip, so a failure is logged and swallowed rather than failing the job — the consumer can always fetch the workflow-artifact bundle. |
...
The signed SLSA VSA (`predicateType: https://slsa.dev/verification_summary/v1`) rides in a per-artifact `<image-basename>-<digest>.intoto.jsonl` bundle. Containers produce no GitHub release, so the **guaranteed** delivery is the **workflow artifact** on every run (npm/Go/Python instead attach the bundle to a release). As a retrieve-by-digest convenience, container builds **additionally** push the bundle to the **registry as a best-effort OCI referrer** on the image digest — so consumers can fetch it from just the image reference when the push succeeded.
