...
2. Wrangle hashes the resulting `.tgz` and hands the hashes to `slsa-framework/slsa-github-generator/.github/workflows/generator_generic_slsa3.yml`. The generator emits an L3 Sigstore-signed in-toto bundle, uploaded as a separate workflow artifact (and on tag pushes, attached as a GitHub Release asset via the generator's `upload-assets`).
...
- The bundle is uploaded as a separate workflow artifact named per the reusable workflow's `provenance-artifact-name` output (format `npm-<shortname>.intoto.jsonl`, namespaced by shortname so multiple npm builds in one workflow don't collide). On tag pushes the bundle is *also* attached as a GitHub Release asset via the generator's `upload-assets`. Consumers verifying after install retrieve the bundle from the GitHub release; non-tag publishes have only the 90-day workflow artifact, which is not retrievable by external consumers.
...
- Verifiable offline via `slsa-verifier verify-artifact --provenance-path <bundle> --source-uri github.com/<owner>/<repo> <tgz>` against the bundle, by any consumer that can fetch the GitHub release's assets.
...
- **Wrangle-owned L3 provenance.** Generated via `generator_generic_slsa3.yml` over the packed `.tgz`, exposed as a separate workflow artifact, and (on tag pushes) attached as a GitHub Release asset. Same pattern as python and container; cross-ecosystem audit tooling reads the same shape regardless of build type.
...
- **Lifecycle hooks.** `package.json`'s `prepare`, `prepack`, `postpack`, `prepublish`, and `prepublishOnly` scripts have different firing semantics, and the threat model differs by hook. By default wrangle runs scripts (the ecosystem norm; common tools like husky and prebuild-install rely on them); adopters who want stricter hardening pass `ignore-scripts: true` to the reusable workflow.
  ...
  - **Opt-in hardening:** `ignore-scripts: true` passes `--ignore-scripts` to both `npm ci` and `npm pack`, suppressing all of the above. The L3 attestation then binds to "what `npm pack` would produce against this source with no script execution." Trade-off: husky's `prepare`-installed git hooks won't be set up (irrelevant for CI), `prebuild-install` binaries won't auto-fetch (consumer-side concern, not wrangle's), and any project relying on `prepack` for transpilation will produce an empty/wrong tarball. Off-by-default because the breakage is significant for typical projects.
...
- **`build.sh` outline:**
  ...
  3. If `package.json`'s `"scripts.build"` exists: `npm run build`. Conditional via `jq has("scripts") and (.scripts | has("build"))` rather than catching `npm run`'s missing-script exit code (clearer logs).
  4. If `"scripts.test"` exists and isn't the default no-op: `npm test`. Default-detection uses substring match against `*'no test specified'*` so minor wording tweaks in future npm releases don't accidentally re-enable the no-op.
  5. `npm pack --pack-destination dist` to produce `<name>-<version>.tgz` (scoped: `<scope>-<name>-<version>.tgz`) directly into `dist/`. Pass `--ignore-scripts` if the `ignore-scripts` input is `true`.
  6. Locate the produced tarball via a glob over `dist/*.tgz`, asserting exactly one match. Channel-free (no `tail -n1` of mixed stdout/stderr) and explicit fail-on-multi catches surprise multi-build scenarios early.
  7. SHA-256 the tarball; emit base64 hashes in the format `slsa-github-generator`'s `base64-subjects` input expects (see python step 5 for the exact `cd dist/` + bare-`*` pattern that keeps subjects as bare filenames so `slsa-verifier` matches them).
  8. `syft dir:<path> -o spdx-json` to produce `sbom.spdx.json` (Cosign-keyless-verified install via `tools/syft/install.sh`, same as python).
  9. OSV-Scanner against the lockfile.
  10. Write the unified `metadata/npm/<shortname>/` directory: `sbom.spdx.json`, `vuln-scan.json`, `summary.md`, `outputs.txt`.
- **Reusable workflow shape mirrors python's:**
  ...
  3. `provenance` job (gated on `should-release`) calls `slsa-framework/slsa-github-generator/.github/workflows/generator_generic_slsa3.yml@vX.Y.Z` with the build's `hashes` output. Re-export the generator's `provenance-name` so the bundle can be downloaded by name. The generator's own `upload-assets: ${{ startsWith(github.ref, 'refs/tags/') }}` attaches the bundle (and dist) as GitHub Release assets on tag pushes.
  4. `verify` job (gated on `should-release && verify-provenance`) downloads dist + provenance, installs `slsa-verifier`, and runs `slsa-verifier verify-artifact --provenance-path <provenance> --source-uri github.com/<owner>/<repo> <tgz>`. Verify failure fails the workflow; the adopter's publish (gated on `needs:`) is blocked.
...
- **Resolved: `npm publish --provenance-file` will not accept generic-generator-built bundles.** Two structural blockers, neither in scope for wrangle to fix. (1) Client-side validation in [`libnpmpublish/lib/provenance.js`](https://github.com/npm/cli/blob/latest/workspaces/libnpmpublish/lib/provenance.js) requires `subject.name` to be a `pkg:npm/<name>@<version>` PURL with `subject.digest.sha512`; `generator_generic_slsa3.yml` emits bare artifact filenames with `sha256`, so the bundle is rejected before it leaves the runner. (2) The npm registry's [server-side validation](https://github.com/npm/provenance/blob/main/README.md) binds the Fulcio signing cert's `Build Signer URI` / `Source Repository URI` extensions to the package's own `repository` field; the generic generator's cert points at `slsa-framework/slsa-github-generator/.github/workflows/generator_generic_slsa3.yml@…`, which won't match. Closing the gap would require upstream changes in either the generic generator (npm-shaped subjects + threading caller repo identity through to Fulcio) or in npm's registry validation. The SLSA Node.js builder is purpose-built to clear both, which is why it's the only documented producer for `--provenance-file` today. **Implication for wrangle:** the Node.js builder remains the only path to populate the registry slot with L3; wrangle's L3 stays in `metadata/npm/<shortname>/` regardless. The design pick (wrangle L3 in metadata + npm L2 in registry slot) is not a hedge — it is the only structurally available option short of importing the Node.js builder's beta envelope.
