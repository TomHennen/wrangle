...
3. Wrangle's metadata artifact (`npm-metadata-<shortname>`) holds the SBOM, the `scan/` tool output, and (on release) the per-artifact `.intoto.jsonl` bundle, per the unified metadata layout. The assembled per-artifact bundles (provenance + signed metadata + VSA) ship inside that metadata artifact.
...
- The attest job assembles one `<artifact>.intoto.jsonl` bundle per subject (the provenance + that subject's signed metadata) and uploads the bundle directory as a transient workflow artifact (`internal-npm-bundles-<shortname>`, namespaced by shortname so multiple npm builds in one workflow don't collide) that the `verify` job downloads to append each VSA; the completed bundles ship in the unified metadata artifact. The provenance is *also* written to GitHub's attestation store (against the caller's repo) for any `should-release` build, so it stays externally verifiable via `gh attestation verify` regardless of trigger. On tag pushes the per-artifact VSA is appended by the `verify:` job and additionally attached as a GitHub Release asset; non-tag publishes have no release-attached VSA.
...
- **Reusable workflow shape mirrors python's:**
  ...
  3. `attest` job (gated on `should-release`) runs `actions/attest-build-provenance` with `subject-path: dist/*`, assembles one `<artifact>.intoto.jsonl` bundle per subject (provenance + signed metadata), and uploads the bundle directory as a transient artifact the `verify` job downloads to append each VSA.
