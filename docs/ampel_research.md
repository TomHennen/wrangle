...
**Revision 14 — 2026-09-20:** Two ampel v1.3.10 behaviour changes the PolicySets and their harness depend on (#833). *Evidence admission hardened:* evidence fetched by a **collector** is dropped whenever it is unsigned or its signature cannot be verified — always, identity gate or not. Unverified evidence is admitted only when handed over explicitly with `--attestation` **and** the policy pins no signer identities, reaching the policy with `verification.verified = false` (`pkg/verifier`: `AdmitUnverified`, `isExplicitEvidence`). Production is unaffected (signed bundles, identity-pinned policies, collectors); `policies/test.bats` now feeds its unsigned logic-only fixtures as `--attestation`, one flag per statement — `--attestation` still parses exactly one statement per file, so a multi-line JSONL bundle must be split. *Tenet outputs are lazy:* an `outputs` entry is evaluated only if the tenet's `code` references it as `outputs.<id>`, so an entry used solely by an `assessment`/`error` template renders empty (`pkg/evaluator/cel/lazy_expr_map.go`). The consumer PolicySets' `levels` output is therefore referenced from `code`, where its empty-predicates guard doubles as the `size(predicates) > 0` check.
