...
Reachability isn't sufficient on its own: a pin can be an ancestor yet still resolve **stale** action code when the path changed after the pin's last bump (#552 — e.g. an inner script edit while the pinned `action.yml` stays byte-identical). `tools/check_pin_freshness.sh` follows the same nested chain and flags any pin whose resolved content (ignoring nested pin-SHA-only differences) differs from `HEAD`. It runs blocking in CI: an action-change PR converges its own pins in-PR (merge commit) so the chain is fresh at merge.

A nested chain (`workflow → verify_release → verify`, `scan → tools/*`) takes one bump cycle per nesting level under squash: a commit can't pin itself, so editing an *inner* action needs the bump repeated until the check is green (#539). Because the check follows nested resolution, it stays red through the intermediate cycles rather than passing on a half-converged chain. Merging the bootstrap-pin PR as a **merge commit** keeps every branch SHA reachable and fresh, so a chain of any depth satisfies those two checks with no re-bump.

Reachable and fresh still isn't the finished state. A converge commit made on the PR branch and brought in by a merge commit sits on the merge's **second-parent** line — an ancestor of main, never on main's first-parent history. Left there, the declared pins are frozen on a branch: both checks above stay green, yet a whole release can ship pinned to non-main history (#592). `tools/check_pin_main_history.sh` closes that gap — it asserts every declared pin whose SHA is on main sits on `origin/main`'s first-parent history and is labeled `# main` (in-flight bootstrap pins, not yet on main, are exempt and left to `check_pin_ancestry`). So the last step of a merge-commit converge is a **finalize bump**: once the merge commit exists, `tools/bump_action_pins.sh <merge-sha>` rolls every pin onto that first-parent commit and relabels them `# main`. Run it detached at `origin/main`, or from any branch — the label comes out `# main` whenever the target is reachable from `origin/main`.
...
3. After a merge-commit converge lands, finalize with `tools/bump_action_pins.sh <merge-sha>` so the declared pins move onto main's first-parent line as `# main` and `check_pin_main_history` goes green.
