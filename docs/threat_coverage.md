...
| Requires a Dependabot config — present, action-pin covered, with an adoption cooldown (wrangle-lint WL001 / WL006 / WL005) | Warns | [chalk/debug phishing](https://www.aikido.dev/blog/npm-debug-and-chalk-packages-compromised), [Shai-Hulud](https://www.cisa.gov/news-events/alerts/2025/09/23/widespread-supply-chain-compromise-impacting-npm-ecosystem), [ua-parser-js](https://github.com/advisories/GHSA-pjwm-rvh2-c87w) | Keeps your dependencies and action pins current — the biggest practical lift — while the cooldown holds back auto-bumps so you don't adopt a freshly-poisoned release inside its live window, before the community flags it | wrangle fails the build on a missing config, missing `github-actions` coverage, or a too-short cooldown, but you can suppress the finding to opt out. It checks the config is wired up — not that every package ecosystem you use is covered — and the cooldown is a delay, not detection: a malicious version still undiscovered after the window can land. |
