...
| `test.bats` | Structural checks (YAML validity, pinned SHAs, required inputs, expected step names, expected `env.ACT` gates). Same pattern wrangle uses for tools (`tools/<name>/test.bats`). | CI |
| `test/caller.yml` + `test/fixtures/` | Action-level behavioral tests run via `nektos/act`. A caller workflow invokes the action's wrapping reusable workflow against a co-located fixture project; the fixture is sized to exercise the specific behavior under test. Full rules — what act can and cannot run, how to handle act-hostile steps, fixture requirements by build type — are specified in [`test/act/SPEC.md`](../test/act/SPEC.md). | CI |
