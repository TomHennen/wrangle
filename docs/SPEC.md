...
The signed VSA (`predicateType: https://slsa.dev/verification_summary/v1`) is the consumer trust boundary: a consumer trusts that one signed attestation instead of re-running the policy engine. Consumers verify it two ways. **`cosign verify-blob-attestation` + a `jq` predicate-field check** is the complete check and the recommended one for npm/go: it binds the signer identity (wrangle's workflow), the **origin repository** (`--certificate-github-workflow-repository`), and the artifact hash, with the `jq` decode covering the predicate fields. **`ampel verify` against the wrangle-hosted `policies/wrangle-vsa-consumer-v1.hjson` PolicySet** is a one-command alternative, but binds only the signer + predicate fields, not the origin repo (a known gap, [#321](https://github.com/TomHennen/wrangle/issues/321)); it is also the signature-checking path for containers, whose VSA subject is an image digest with no blob for cosign. `slsa-verifier verify-vsa` is **not** usable: it requires `--public-key-path` and verifies only key-signed VSAs, while wrangle's are keyless (Fulcio/Sigstore). Per-build-type commands live in each `build/actions/<type>/README.md`.
