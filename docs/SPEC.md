...
Action-level behavioral testing — exercising the reusable workflow end-to-end against a real fixture project — happens in a dedicated integration-testing repo rather than inside this repo. That setup is specified separately (see the integration-testing spec, not yet written as of this pass). The short version: a companion repo runs each wrangle reusable workflow against a fixture, triggered per-PR for internal contributors and on merge to main for broader coverage.
