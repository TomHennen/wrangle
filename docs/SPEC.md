...
Every build job produces two consumer-facing workflow artifacts: `<type>-dist-<sn>` (the product — npm/go/python; container ships its product to the registry instead) and `<type>-metadata-<sn>` (the unified set above). The rest are internal job-passing transients (scan output, the attest job's assembled bundles, the build job's pre-verify metadata) that are folded into `<type>-metadata-<sn>` and not part of the consumer contract.

The `attest` job assembles the per-artifact bundles, one `<artifact>.intoto.jsonl` per dist subject (the shared SLSA provenance + that subject's signed SBOM + scan/v1 statements), following the [in-toto bundle spec](https://github.com/in-toto/attestation/blob/main/spec/v1/bundle.md) naming so several build types publishing to one release surface can't collide. One `wrangle-attest assemble` run signs every subject's metadata and writes every bundle, so the whole job needs a single keyless signing flow. Signing every build-metadata attestation here keeps the trust boundary clean: the `attest` job runs no adopter-controlled code — only `attest-build-provenance` and `wrangle-attest` over the already-built dist/image, never an adopter build or test hook (see [wrangle-attest/SPEC.md](../tools/wrangle-attest/SPEC.md)). For npm/go/python the job hands the engine `actions/attest-build-provenance`'s bundle as the provenance seed; for container it hands it the image's registry referrers (`cosign download attestation`), which the engine filters to the SLSA provenance envelope. The bundle directory is passed to the `verify` job as a transient workflow artifact namespaced by build type and shortname (`<type>-bundles-<shortname>`) so multiple builds in one workflow don't collide — `actions/download-artifact` picks non-deterministically when two artifacts share a name in the same run, and the verify job reading the wrong build's bundles would fail with a confusing subject mismatch. This transient is internal job-passing, not part of the consumer contract. The `verify` job's only remaining bundle responsibility is to append each subject's signed VSA to the matching bundle (see [Release verification](#release-verification)); it writes the completed bundles into `metadata/<type>/<sn>/` and uploads the whole directory as `<type>-metadata-<sn>` (exposed via `metadata-artifact-name`); for npm/go/python they are additionally attached to the release as `<artifact>.intoto.jsonl`, and for container the VSA is also pushed as its own by-digest registry referrer.
...
| **Go: `govulncheck` traces a call to a vulnerable symbol** | Pipeline stops unless the module's `govulncheck-ignore.toml` suppresses the id, or the adopter sets `govulncheck: info`. Findings govulncheck can't trace to a symbol are warnings. | A proven call path is the most precise vulnerability signal wrangle has — more precise than the lockfile scan that already gates at `:fail`, and it is the only one that sees the Go stdlib. Semantics in [`build/actions/go/SPEC.md`](../build/actions/go/SPEC.md) "Failure semantics". |
...
1. ampel collects the just-produced provenance — the attest job's signed bundle for npm/go/python (passed as a workflow artifact), or the image's OCI 1.1 referrer for container — and evaluates it against the PolicySet named by the workflow's `policy` input. The default is the build type's `wrangle-default-<type>-v1` tier (provenance + SBOM-present + OSV/zizmor/wrangle-lint scan-clean); `wrangle-provenance-<type>-v1` (provenance-only) and `wrangle-strict-<type>-v1` (default + Scorecard ≥ 7.0) are the lighter and heavier siblings. The scan tenets read wrangle's `scan/v1` envelope (`data.tool.name`, `data.result`) and fail closed on a missing attestation, so an adopter who drops osv/zizmor/wrangle-lint from `scan-tools` must select a lighter `policy`.
...
The signed VSA (`predicateType: https://slsa.dev/verification_summary/v1`) is the consumer trust boundary: a consumer trusts that one signed attestation instead of re-running the policy engine. The recommended check for every build type is a one-command **`ampel verify`** (ampel ≥ v1.3.0) against the wrangle-hosted `policies/wrangle-vsa-consumer-v1.hjson` PolicySet: it binds the signer identity (wrangle's reusable workflow, pinned at a `@refs/tags/vX.Y.Z` release tag — the policy requires the tag-ref form, so a SHA-pinned build's VSA does not verify), the **origin repository** — the policy's `sourceRepositoryUriMatch` matches the signing cert's source-repository extension against the consumer-supplied `sourceRepo` context, whole-value and fail-closed ([#321](https://github.com/TomHennen/wrangle/issues/321)) — the artifact hash, and the predicate fields. Both context values (`expectedResourceUri`, `sourceRepo`) are required, so omitting one is a hard error, never a weaker check; pre-1.3.0 ampel fails to parse the policy rather than silently skipping the repo binding. cosign is the equivalent-strength alternative: for npm/go/python, `cosign verify-blob-attestation` + a `jq` predicate-field decode; for containers — where the VSA subject is an image digest with no file blob — `cosign verify-attestation` (cosign v3) against the by-digest VSA OCI 1.1 referrer (not the `.att`-tag SLSA provenance), or `cosign verify-blob-attestation` against the combined workflow-artifact bundle's VSA line; both forms bind the origin repository via `--certificate-github-workflow-repository`. The integration suite's `verify-container-vsa` job and `test/consumer/verify_consumer_vsa.bats` are the regression backstops. `slsa-verifier verify-vsa` is **not** usable: it requires `--public-key-path` and verifies only key-signed VSAs, while wrangle's are keyless (Fulcio/Sigstore). Per-build-type commands live in each `build/actions/<type>/README.md`.

The verifiable identity is the VSA's **signing certificate — wrangle's reusable workflow** (`build_and_publish_<type>.yml`); the provenance's `builder.id` now names that same workflow (below), and `verifier.id` (`https://carabiner.dev/ampel@v1`, the engine) is hard-coded and not wrangle. So consumers pin wrangle's workflow path as the cert identity and pass their own repo — the `sourceRepo` context on the ampel path, `--certificate-github-workflow-repository` on the cosign path.
...
jobs:
  check-change:
    ...
    uses: TomHennen/wrangle/.github/workflows/check_source_change.yml@v0.4.1 # zizmor: ignore[unpinned-uses] - immutable
```
...
Every wrangle reusable workflow runs the `prep` job at the head of `jobs:`, whose first step runs [`actions/prep/preflight_guard.sh`](../actions/prep/preflight_guard.sh). Refusal fails the workflow; every other job declares `needs: [prep]` so a refused invocation skips the entire run — no OIDC tokens minted, no privileged actions executed, no docker push, no provenance generation.
...
| | `actions/prep/preflight_guard.sh` | `actions/prep/release_gate.sh` |
|---|---|---|
...
- [ ] [Ampel](https://github.com/carabiner-dev/ampel) integration — policy
      ...
      post-v1.0 work item.
- [x] Adopter-side `verify-vsa` action (`actions/verify-vsa/`) — closes the runner-isolation gap between wrangle's `verify` job and the adopter's publish job by verifying the publish job's own downloaded bytes against the signed VSA, the full policy verdict (chosen over raw provenance so the gate inherits every PolicySet tenet as the policy grows; the VSA only exists on a PASS, so a wired-out verify job also fails closed). Per file: one `ampel verify` against the `wrangle-vsa-consumer-v1` PolicySet, binding subject hash, signer identity, origin repo (`sourceRepo` context), the expected `resourceUri`, and the PASSED/L3 verdict — the same one-command check downstream consumers run. Wired into the npm and python examples. Downstream-consumer verification remains the per-build-type README commands. Tracking: [#198](https://github.com/TomHennen/wrangle/issues/198)
- [x] Merge wrangle attestations into per-artifact in-toto JSONL bundles across all build types — the `attest` job assembles one `<artifact>.intoto.jsonl` per dist subject (provenance + that subject's signed metadata) and the `verify` job appends each subject's VSA, delivered as release assets (npm/go/python) or, for the single container subject, the workflow artifact plus the VSA pushed as its own by-digest OCI referrer. Tracking: [#181](https://github.com/TomHennen/wrangle/issues/181)
...
- [x] Divergence-fail guard for the duplicated `govulncheck` version literal
      (`test/Dockerfile` ↔ `build/actions/go/checks` default), mirroring the
      zizmor parity guard — `test/test_govulncheck_pin_consistency.bats`.
      Tracking: [#286](https://github.com/TomHennen/wrangle/issues/286)
