...
For npm/go/python the signed provenance bundle is uploaded as a workflow artifact namespaced by build type and shortname (`<type>-provenance-bundle-<shortname>`) so multiple builds in one workflow don't collide — `actions/download-artifact` picks non-deterministically when two artifacts share a name in the same run, and the verify job reading the wrong build's bundle would fail with a confusing subject mismatch. The artifact name is exposed via the workflow's `provenance-artifact-name` output so adopters and downstream consumers don't need to reconstruct the convention themselves. (The container attestation is a registry referrer on the image digest, not a workflow artifact.) This bundle is an internal intermediate; the `verify` job consumes it to build the consumer-facing `multiple.intoto.jsonl` (provenance + VSAs — see [Release verification](#release-verification)), uploaded under the same namespacing (`<type>-bundle-<shortname>`, exposed via `bundle-artifact-name`) and attached to the release as `multiple.intoto.jsonl`.
...
4. On a PASS, the same job signs one VSA per dist subject and appends them to the provenance to emit a single `multiple.intoto.jsonl` bundle (a valid in-toto JSONL bundle — one statement per line: the signed SLSA provenance lines plus one signed VSA per subject) — so verification and the consumer artifact are one step, and the wrangle signer identity is declared once (in the PolicySet) rather than duplicated in a separate verify flag. The unsigned VSA never crosses a step boundary: emit → `bnd`-sign → append happens in one `actions/verify` process. The bundle is the single delivery vehicle for every wrangle-produced attestation — delivered as a GitHub release asset for npm/go/python and as one OCI 1.1 referrer (built in-job by `cosign download attestation` → append → `cosign attach attestation`) for container. This is issue [#181](https://github.com/TomHennen/wrangle/issues/181)'s consolidation; per-subject VSA semantics are preserved (one VSA statement per subject), just packaged in one file.
...
- [x] Bundle wrangle attestations into a single in-toto JSONL across all build types — the `verify` job emits one `multiple.intoto.jsonl` (provenance + one VSA per dist subject) delivered as a release asset (npm/go/python) or OCI referrer (container). Tracking: [#181](https://github.com/TomHennen/wrangle/issues/181)
