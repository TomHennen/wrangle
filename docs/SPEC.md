...
4. On a PASS, the same job signs one VSA per dist subject and appends it to the provenance to emit one `<artifact>.intoto.jsonl` bundle per subject (each a valid in-toto JSONL bundle — one statement per line: the signed SLSA provenance lines plus that subject's signed VSA) — so verification and the consumer artifact are one step, and the wrangle signer identity is declared once (in the PolicySet) rather than duplicated in a separate verify flag. Per-artifact names follow the [in-toto bundle spec](https://github.com/in-toto/attestation/blob/main/spec/v1/bundle.md) and can't collide when several build types publish to one release. The unsigned VSA never crosses a step boundary: emit → `bnd`-sign → append happens in one `actions/verify` process. The bundles are the delivery vehicle for every wrangle-produced attestation, and the file delivery is uniform across build types: every per-artifact bundle is uploaded as a workflow artifact and attached to the GitHub release when one exists for the tag. A container build has a single image-digest subject, so it produces one bundle; in addition to the file delivery it is pushed to the registry as an OCI 1.1 referrer on the image digest, best-effort (`cosign download attestation` seeds the provenance → append → `cosign attach attestation` pushes the bundle, logged-and-skipped on failure). `cosign attach attestation` accepts a single per-artifact bundle line and round-trips it verbatim, but rejects a multi-line concatenation, so each bundle is pushed as its own referrer; the file bundle is the guaranteed path, so the registry push never fails the job. This is issue [#181](https://github.com/TomHennen/wrangle/issues/181)'s consolidation; per-subject VSA semantics are preserved (one VSA statement per subject, merged with the provenance), now packaged one file per artifact.
...
- [x] Merge wrangle attestations into per-artifact in-toto JSONL bundles across all build types — the `verify` job emits one `<artifact>.intoto.jsonl` per dist subject (provenance + that subject's VSA), delivered for every build type as a workflow artifact and a release asset (when a release exists); container additionally pushes each bundle as a best-effort OCI referrer. Tracking: [#181](https://github.com/TomHennen/wrangle/issues/181)
