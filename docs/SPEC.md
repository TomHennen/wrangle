...
| **Cosign signing fails** | Build continues with a warning. Unsigned artifact is published. | Signing failures are usually infrastructure issues (Sigstore/Fulcio unavailability). Blocking the build on signing infra would create flaky releases. |
| **SLSA provenance generation fails** | Build continues with a warning. Artifact is published without provenance. | Same rationale as signing — provenance is generated by an external service (`slsa-github-generator`). |

This "fail on code quality, warn on infrastructure" approach keeps the pipeline reliable while surfacing all security information.
