...
echo "Generate sarif"
/osv-scanner --format sarif --output /metadata/osv_sbom/output.sarif --sbom $1 || WRANGLE_EXIT_STATUS=1
# Run it again for the markdown. (if only we could output in multiple formats from one run...)
echo "Generate markdown"
/osv-scanner --format markdown --output /metadata/osv_sbom/output.md --sbom $1 || WRANGLE_EXIT_STATUS=1
