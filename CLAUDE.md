...
Always run `./test.sh` before pushing — CI runs the same checks. `./test.sh quick` skips zizmor for inner-loop iteration. No Docker? `make test` runs the same checks directly on the host; install the toolchain `test/Dockerfile` describes (same pins) first. Every adapter and install script gets a `test.bats`. The test-layer breakdown (local layers, the containerized unit suite, CI integration, e2e) is in SPEC.md §Testing Strategy. Beyond that:
