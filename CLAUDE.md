...
- **One exception:** a PR whose only changed file is `tools/catalog.json` may merge on green CI, because the `catalog-bump` check (`tools/check_catalog_bump_pr.sh`) proves it is byte-identical to what `tools/bump_catalog_to_latest.sh` produces against the registry's current `:latest`.
