...
- Cross-repo consumption (`uses: tomhennen/wrangle/...@v0.2.1 # zizmor: ignore[unpinned-uses] - immutable`) is only testable after tagging a release. If your change affects the reusable workflow interface, note this in the PR description.
...
- Do not use `@main` in production — pin the release tag (e.g., `@v0.2.1 # zizmor: ignore[unpinned-uses] - immutable`); wrangle's release tags are immutable, and the inline ignore silences the `unpinned-uses` finding that can't tell. A SHA pin works too if you prefer (no ignore needed)
