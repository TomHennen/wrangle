...
wrangle_validate_verify_attestation_inputs() {
    if [[ $# -ne 5 ]]; then
        printf 'Usage: wrangle_validate_verify_attestation_inputs SUBJECT REPO SIGNER_WORKFLOW BUNDLE_PATH PREDICATE_TYPE\n' >&2
        return 1
    fi

    local subj="$1" repo="$2" signer_workflow="$3"
    local bundle_path="$4" predicate_type="$5"
